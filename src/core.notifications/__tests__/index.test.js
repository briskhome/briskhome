...
describe('core.notifications', () => {
  ...
  describe('#define()', () => {
    ...
    it('returns false when unable to fetch events', async () => {
      ...
      expect(log.error).toHaveBeenCalledWith(
        { err: mockError, data: mockPayload },
        ERR_UNABLE_TO_FETCH,
      );
    });
  ...
  describe('#evaluate()', () => {
    ...
    it('logs when no event fetched', async () => {
      ...
      await component.evaluate(mockEvent);
      expect(log.debug).toHaveBeenCalledWith({ data: mockEvent }, ERR_NO_EVENT);
    });
    ...
    it('skips when user is not subscribed to level', async () => {
      ...
      fetchBySubscription.mockReturnValue(
        Promise.resolve([mockUserNoContacts]),
      );
      await component.evaluate(mockEvent);
    });
    ...
    it('logs when user has no contacts', async () => {
      ...
      expect(log.warn).toHaveBeenCalledWith(
        { data: { user: mockUserNoContacts, event: mockEvent } },
        ERR_NO_CONTACTS,
      );
    });
