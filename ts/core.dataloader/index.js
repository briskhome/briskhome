...
export default ((imports: CoreImports) => {
  ...
  const createDataloaders = () => {
    const batchLoadDevicesById = async (ids: Array<string>) => {
      log.debug({
        dataloader: 'deviceById'
      }, 'batchLoadDeviceById', {
        count: ids.length,
        ids
      });
      return Promise.all(ids.map(id => Device.findOne({
        _id: id
      }).exec()));
    };
    ...
    const batchLoadSensorsById = async (ids: Array<string>) => {
      log.debug({
        dataloader: 'sensorById'
      }, 'batchLoadSensorById', {
        count: ids.length,
        ids
      });
      return Promise.all(ids.map(id => Sensor.findOne({
        _id: id
      }).exec()));
    };
    ...
    const batchLoadUsersById = async (ids: Array<string>) => {
      ...
      return Promise.all(ids.map(id => User.findOne({
        _id: id
      }).exec()));
    };
