    ...
    values: {
      ...
      resolve: async (src, args, ctx) => {
        ...
        const query = await ValueModel.find({
          sensor: src._id,
          timestamp: {
            $gte: moment(args.from).utc().startOf('day'),
            $lte: moment(args.to).utc().endOf('day')
          },
          'values.type': type,
          'values.timestamp': {
            $gte: moment(args.from).utc().toDate(),
            $lte: moment(args.to).utc().toDate()
          }
        }).exec();
        if (!query.length) return null;
        return query.reduce((acc, day) => acc.concat(day.values.map(v => ({
