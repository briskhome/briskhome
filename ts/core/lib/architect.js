...
class Architect extends events.EventEmitter {
  ...
  load = async (type: string, options: {
    args: Array<any>;
    exec: boolean;
  }) => {
    log.debug({
      type
    }, `Resolving extensions from config`);
    const extensions = await this.resolve(type);
    log.debug({
      type
    }, `Resolved ${extensions.length} extensions`);
    const failed = [];
  ...
  register = async (extension: any, {
    ...
    try {
      ...
      if (extension.name in instance) instance = instance[extension.name];else instance = instance['default'];
