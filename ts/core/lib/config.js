...
import { flatten, isExtension, isPathname, normalize, timeout, toPathname } from "../utils";
import { Extensions, PackageJson } from "../types";
...
class Config {
  ...
  async discover() {
    ...
    const buckets = await Promise.all(this.directories.map(directory => readdirAsync(directory)));
    const packages = flatten(buckets.map((directories, index) => directories.map(dir => path.resolve(this.directories[index], dir))));
  ...
  async prepareExtensions(directories: Array<string>, base: string) {
    const modules: Array<object> = await Promise.all(directories.map(directory => Promise.race([this.getMetadata(base, directory), timeout()])));
    const extensions = flatten(modules.filter(extension => !(extension instanceof ArchitectError)));
  ...
  async getMetadata(base: string, name: string): Promise<any> {
    let metadata: PackageJson | undefined | null = null;
    let extensionMain: string | undefined | null = null;
    ...
    try {
      if (!metadata) {
        ...
        const dir: string | undefined | null = path.resolve(base, name);
    ...
    if (!isExtension(metadata)) {
      return new ArchitectError('Resolved package is not a valid extension', 'ERR_INVALID_EXTENSION');
    }
  ...
  async getRealpath(base: string, name: string): Promise<string> {
    if (isPathname(name)) {
      ...
      try {
        ...
        throw new ArchitectError(`Can't resolve extension '${name}' relative to '${base}'`, 'ERR_NO_EXTENSION');
      }
    }
    ...
    const next = async (maybeBase: string): Promise<string> => {
      ...
      try {
        ...
        if (nextBase === maybeBase) {
          throw new ArchitectError(`Can't resolve package '${name}' relative to '${base}'`, 'ERR_NO_PACKAGE');
        }
    ...
    function parse(extension, type) {
      ...
      if (typeof extension === 'string') {
        ...
        parsed.push({
          ...
          type,
          main: extension.main ? path.resolve(metadata.dir, extension.main) : metadata.main,
