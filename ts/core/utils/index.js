...
export const isExtension = (obj: object) => {
  return obj.extensions && obj.peerDependencies && obj.peerDependencies['@briskhome/core'];
}; //

const EXT_TIMEOUT = 'Extension failed to load within 1000ms timeout';
export const timeout = (t: number = 1000): Promise<any> => {
  return new Promise((unused, reject) => setTimeout(() => reject(new ArchitectError(EXT_TIMEOUT, 'EXT_TIMEOUT')), t));
}; //

export const normalize = (name: string): string => {
  let normalizedName: string = name; // if (name.indexOf('core.') >= 0) {
...
export const maybeExtension = async (dir: string): Promise<boolean> => {
  ...
  return stats.isDirectory() && (dir.includes('core.') || dir.includes('briskhome-'));
};
export const write = (err: string) => {
  const writeln = (message, data = {}) => process.stdout.write(JSON.stringify({
    name: 'briskhome',
