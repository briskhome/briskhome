...
export class UserCard extends React.Component<UserCardProps, UserCardState> {
  ...
  renderContent: Function = () => {
    ...
    if (!users.length) {
      return <span className="card__content_empty">No users are registered.</span>;
    }

    return <div>
        <table className="widget-table">
          <tbody>
            {users && users.map(user => <tr className="widget-table__row">
                  <td className="widget-table__cell widget-table__cell_first">
                    <Avatar name={user.name.split(' ').map(el => el[0]).join('')} online />
                  </td>
                  ...
                  <td className="widget-table__cell widget-table__cell_last">
                    <Menu arrow options={[<a className="link" href="#">
                          More info
                        </a>, <a className="link link_red" href="#" onClick={() => mutate({
                variables: {
                  username: user.id
                },
                refetchQueries: [{
        ...
        <Modal isOpen={this.state.addUserModalOpen} onRequestClose={this.toggleAddUserModal} closeTimeoutMS={250} contentLabel="AddUserModal" className={{
  ...
  render() {
    ...
    return <Card error={error} loading={loading} title="Users & Guests" button="Add" onClick={this.toggleAddUserModal} caption="No users online">
        {this.renderContent()}
      </Card>;
  }
