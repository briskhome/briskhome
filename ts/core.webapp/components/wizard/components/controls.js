...
export const WizardControls = ({
  ...
  return <div className={cn('briskhome-wizard__footer-controls', {
    ...
    'briskhome-wizard__footer-controls_justify': prev && next
      ...
      {prev && <Button link="" display="inline-block" className={cn('briskhome-wizard__button-prev', {
      'briskhome-wizard__button_hidden': !prev
    }, prevClassName)} onClick={prev}>
          {prevLabel}
        </Button>}
      {next && <Button caps link="" large yellow loading={false} display="inline-block" className={cn('briskhome-wizard__button-next', {
      'briskhome-wizard__button_hidden': !next
    }, nextClassName)} onClick={next}>
          {nextLabel}
        </Button>}
    </div>;
};
