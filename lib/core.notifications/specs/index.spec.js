...
describe('core.notifications', () => {
  ...
  describe('#register()', () => {
    ...
    it('returns false when unable to fetch events', async () => {
      ...
      const res = await component.register(mockData);
      expect(res).toEqual(false);
      expect(log.error)
        .toHaveBeenCalledWith({ err: mockError, data: mockData }, ERR_UNABLE_TO_FETCH);
    });
    ...
    it('returns false when unable to save event', async () => {
      ...
      expect(log.error)
        .toHaveBeenCalledWith({ err: mockError, data: mockData }, ERR_UNABLE_TO_REGISTER);
    });
    ...
    it('updates existing event definition when one exists', async () => {
      ...
      expect(log.debug)
        .toHaveBeenCalledWith({ data: mockData }, EVENT_REGISTER_SUCCESS);
    });
  ...
  describe('#subscribe()', () => {
    ...
    beforeEach(() => {
      ...
      bus.removeAllListeners(mockData.event.id);
    });

    it('returns false when unable to subscribe user', async () => {
      fetchByUsername.mockReturnValue(Promise.reject(mockError));
      const res = await component.subscribe(mockData);
      expect(res).toEqual(false);
      expect(log.error)
        .toHaveBeenCalledWith({ err: mockError, data: mockData }, ERR_UNABLE_TO_SUBSCRIBE);
    });
  ...
  describe('#unsubscribe()', () => {
    ...
    it('returns false when unable to fetch user', async () => {
      fetchByUsername.mockReturnValue(Promise.reject(mockError));
      const res = await component.unsubscribe(mockData);
      expect(res).toEqual(false);
      expect(log.error)
        .toHaveBeenCalledWith({ err: mockError, data: mockData }, ERR_UNABLE_TO_UNSUBSCRIBE);
    });
  ...
  describe('#evaluate()', () => {
    ...
    it('logs when unable to fetch user', async () => {
      ...
      expect(log.error)
        .toHaveBeenCalledWith({ err: mockError }, ERR_UNABLE_TO_FETCH);
    });
    ...
    it('logs when no subscribers fetched', async () => {
      ...
      await component.evaluate(mockEvent);
      expect(log.debug)
        .toHaveBeenCalledWith({ data: mockEvent }, ERR_NO_SUBSCRIBERS);
    });
